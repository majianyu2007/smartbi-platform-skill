...
本 Skill 默认把 Smartbi 当作主要数据处理环境，而不是只用于展示本地
加工好的结果。本地操作原则上仅限于数据集检索、下载、授权与完整性检查；
平台不支持原始文件容器时，才允许做不改变记录含义的最小格式转换。

筛选、缺失值处理、去重、关联、派生指标、聚合、透视和排序必须优先通过
Smartbi ETL 节点完成。`关系数据源 → 覆盖到关系表` 的直接复制，以及只增加
序号的流程，不能作为竞赛 ETL 的完成证据。数据模型、仪表盘和 AIChat 应
读取 ETL 物化输出；本地计算只用于独立复核平台结果。

### 候选数据集隔离原则

当比赛最终只能选用一个数据集时，每个候选数据集必须拥有独立目录、ETL、
输出表、模型、分析、仪表盘，以及本轮要求创建的 AIChat 图谱。候选数据集
之间不得在筛选阶段做追加、合并或关联，也不得建立跨数据集分析模型。

候选比较只写入外部评分表或不含结果值的元数据记录。验收时逐个核对 ETL
终态与行数、模型单表血缘、分析可执行性和仪表盘实际渲染；一个数据集验收
通过不能替代其他数据集的验收。

### 初学者界面与 Skill 命令对应

- `数据连接 → 文件 → 新建数据表` 对应 `upload`；
- `数据准备 → 自助ETL` 对应 `etl-*`，`数据准备 → 数据集`
  （首页卡片显示“数据模型”）对应 `model-*`；
- `分析展现 → 透视分析` 对应 `analysis-*`，`交互式仪表盘`
  对应 `dashboard-*`；
...
坐标轴、布局和字段绑定。最终必须在有头浏览器中重新打开并目视检查。
